...
fn semicolon_style_always_inserts_missing() {
    ...
    let out = format_document("function f() { let x = 1 }", Version::V4, &opts).unwrap();
    assert!(
...
fn formats_arrow_lambda_without_duplicate_arrow() {
    let out = format_document("let f = x -> x + 1;", Version::V4, &FormatOptions::default()).unwrap();
...
fn file_wide_inner_directive_formats_lines_above() {
    ...
    let out_outer = format_document(src_outer, Version::V4, &FormatOptions::default()).unwrap();
    assert!(
...
fn spaces_around_keyword_binary_ops_before_paren() {
    ...
    let out = format_document("let x=(a as integer);", Version::V4, &FormatOptions::default()).unwrap();
    assert!(
...
fn spaces_after_assign_before_paren() {
    let out = format_document("let x=(y);", Version::V4, &FormatOptions::default()).unwrap();
    assert!(
...
fn space_after_comma_default() {
    let out = format_document("let x = f(1,2);", Version::V4, &FormatOptions::default()).unwrap();
    assert!(
        out.contains("f(1, 2)"),
...
fn space_after_comma_false() {
    ...
    let out = format_document("let x = f(1, 2);", Version::V4, &o).unwrap();
...
fn comparison_and_bitwise_still_spaced_outside_types() {
    let out = format_document("let x = 1 | 2;", Version::V4, &FormatOptions::default()).unwrap();
    assert!(
        out.contains("1 | 2"),
