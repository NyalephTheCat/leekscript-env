...
impl WorldMap {
    ...
    pub fn verify_los(
        ...
        let Some(s) = self.get_cell(start) else {
            return false;
        };
        let Some(e) = self.get_cell(end) else {
            return false;
        };
        ...
        for p in (0..path.len()).step_by(2) {
            ...
            for i in 0..seg_len {
                ...
                let Some(cid) = self.get_cell_xy(cx, cy) else {
                    return false;
                };
                let Some(cell) = self.get_cell(cid) else {
                    return false;
                };
                if !cell.walkable {
                    return false;
    ...
    pub fn compute_components(&self) -> Vec<i32> {
        ...
        for id in 0..self.nb_cells {
            ...
            let Some(c0) = self.get_cell(id) else {
                continue;
            };
            ...
            while let Some(cur) = q.pop_front() {
                for n in self.neighbors4(cur) {
                    ...
                    let Some(nc) = self.get_cell(n) else {
                        continue;
                    };
    ...
    pub fn random_cell(&self, rng: &mut Rng, occupied: &HashSet<i32>) -> Option<i32> {
        ...
        loop {
            ...
            out = Some(rng.int_inclusive(0, self.nb_cells - 1));
            if nb > 64 {
                break;
            }
            nb += 1;
    ...
    pub fn a_star_path(
        ...
        let in_bounds = |id: i32| id >= 0 && id < self.nb_cells;
        if !in_bounds(start) || end_cells.iter().copied().any(|id| !in_bounds(id)) {
            return None;
        }
    ...
    pub fn generate_random(
        ...
        while !valid && nb < 63 {
            ...
            for _ in 0..obstacle_count {
                ...
                let cid = rng.int_inclusive(0, map.nb_cells - 1);
                let Some(c) = map.get_cell(cid) else {
                    continue;
                };
